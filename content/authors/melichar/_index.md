---
# Display name
title: Gustav Melichar

# Full name
first_name: Gustav
last_name: Melichar

# Status emoji (optional, delete the line if you don't want one)
# status:
#   icon: 

# Is this the primary user of the site?
superuser: true

# Role/position
role: Postdoctoral Researcher, Philosophical Counselor & Habilitation Candidate (University of Heidelberg)

# Organizations/Affiliations
organizations:
  - name: Heidelberg University Hospital
    url: 'https://www.klinikum.uni-heidelberg.de'

# Short bio (shown in author boxes)
bio: I work at the intersection of Aristotelian naturalism, enactivist cognitive science, and transcendental philosophy.

# Social/Academic Networking
profiles:
  - icon: brands/linkedin
    url: https://www.linkedin.com/in/gustav-melichar-617732152/
  - icon: academicons/academia
    url: https://uni-heidelberg.academia.edu/GustavMelichar

# Interests
interests:
  - Philosophy of Psychology and Psychiatry
  - Philosophy of Religion
  - Aristotelian Naturalism
  - Enactivist Cognitive Science
  - Transcendental Philosophy
  - History of Philosophy

# Education
education:
  - area: PhD Philosophy
    institution: Eberhard-Karls-Universität Tübingen
    date_end: '2019'
  - area: Magister Artium Philosophy
    institution: Friedrich-Schiller-Universität Jena
    date_end: '2012'

# highlight the author in author lists? (true/false)
highlight_name: true

# Avatar image (place image in `assets/media/` folder)
image:
  filename: IMG_1199.jpeg
---

I am a postdoctoral researcher at the Section for Phenomenological Psychopathology and Psychotherapy, Heidelberg University Hospital, and a trained philosophical counselor.

I work at the intersection of Aristotelian naturalism, enactivist cognitive science, and transcendental philosophy to develop a non-reductive account of embodied rational capacities. My research addresses the normativity of mental illness, the conditions of autonomy, and the role of trust in social and therapeutic practices, drawing systematically on both the history of philosophy and contemporary cognitive science.
